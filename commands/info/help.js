  ...
  async execute(client, message, args, ee, prefix) {
    ...
    try {
      if (args[0]) {
        ...
        if (cmd.aliases) try {
          embed.addField("**⬆️**", `\`\`\`${cmd.aliases.map((a) => `${a}`).join("`, `")}\`\`\``);
        } catch {}
        if (cmd.cooldown) embed.addField("** ⬆️Cooldown**", `\`\`\`${cmd.cooldown} Seconds\`\`\``);
        if (cmd.usage) {
        ...
        // Main Buttons
        let button_home = new MessageButton().setStyle('SUCCESS').setCustomId('Home').setEmoji("937952762765914192")
        ...
        let OverviewEmbed = new MessageEmbed()
          ...
          .setFooter("Thx for using me",`https://images-ext-2.discordapp.net/external/wjzN1Pi7whWSGbXgrA-3YAYUjg8PxPBubBdfNphwGCg/%3Fsize%3D44%26quality%3Dlossless/https/cdn.discordapp.com/emojis/936918365304414219.gif`)
...
${client.allEmojis.help.report} Report**`,true)
        .addField(`${client.allEmojis.help.cate} **STATS**`,`>>> ${client.allEmojis.arrow}**Guilds:** \`${client.guilds.cache.size}\`\n${client.allEmojis.arrow}**Members:** \`${client.guilds.cache.reduce((acc, guild) => acc + guild.memberCount, 0)}\`\n${client.allEmojis.arrow}**Prefix Commands:** \`${client.commands.map(a => a).length}\`\n${client.allEmojis.arrow}**Slash Commands:** \`${client.slashCommands.map(a => a).length}\`\n${client.allEmojis.arrow}**Invite:** [**${client.user.username}**](https://discord.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot%20applications.commands)\n${client.allEmojis.arrow}**Hosted By:** [**Night Lounge**](https://discord.gg/)` , true)
